SimpleTranslator = {
    ...
    translate: function () {
        ...
        if (locale == 'ko' || locale == 'koKR' || locale == 'ko-KR') {
            $("#lblSbBigTitle").html('PagePostManager v3 :: 페이스북용 페이지 게시물 매니저');
            ...
            $("#sb-intro").text("PagePostManager를 이용하여 페이지의 좋아요와 댓글을 손쉽게 추출할 수 있습니다.");
            $("#login-required").text("실행을 위해서는 페이스북에서 앱 사용을 허가해야 합니다.");
            $("#btn-basic-login").text("로그인하고 시작하기");

            $("#sb-show-admin").text("내가 관리하는 페이지 보기");
            ...
            $("#btn-board-search-again").text("페이지 검색으로 돌아가기");
            $("#board-name").text("게시물 목록");
            $("#btn-board-post-list-more").text("더 보기");
            $('#manual-search').html(
                "<b>2019-11-03 현재,</b><u>페이스북 비즈니스 계정</u>으로 로그인하신 경우, 목록이 제대로 표시되지 않을 수 있습니다."
                + "<br/>이 경우에는, 포스팅 추출을 원하는 페이지 이름(예:PagePostManager)을 아래 칸에 입력해주세요.");
            $('#btn-manual-search').text("수동으로 찾기");

            $("#btn-goto-search").text("페이지 검색으로 이동");
            ...
            $("#lbl-trouble-login").text("로그인하시는 데 문제가 있다면, 다음 방법을 시도해 보세요.");
            $("#btn-generate-bug-report").text("개발자에게 도움 요청(진단 리포트 작성)");
        }
    },
    getKey: function (key) {
        if (locale == 'ko' || locale == 'koKR' || locale == 'ko-KR') {
