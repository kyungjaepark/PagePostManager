...
function getResultTableXlsx(tableGenerationOption) {
    if (tableGenerationOption["tableType"] == 'comments') {
        return getCommentsXlsx(
            tableGenerationOption["commentsArray"],
            tableGenerationOption["reactionsMap"]
            , chkShowAttachment.checked, chkReactions.checked, chkCommentLink.checked,
            $('#chkShowTopInfo').prop('checked'));
    ...
    if (tableGenerationOption["tableType"] == 'reactions') {
        return generateReactionsXlsx(g_appContext.tableGenerationOption["reactionsMap"]);
    }
    return '';
}

function generateReactionsXlsx(reactionsMap) {
    return null;
    var stringBuilder = [];
    stringBuilder.push("<tr><th>ID</th><th>이름</th><th>반응</th></tr>");
    for (var x in reactionsMap)
...
function getCommentsXlsx(commentsArray, reactionsMap, isShowAttachment, isShowReactions, isShowCommentLink, isShowTopInfo) {
    return null;
